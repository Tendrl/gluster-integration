...
class ValidateExpandVolumeInputs(objects.BaseAtom):
    ...
    def run(self):
        logger.log(
            "info",
            NS.publisher_id,
            {"message": "Checking if inputs for expand"
             " vol are valid for %s" % self.parameters[
                 'Volume.volname'
             ]},
        ...
        if replica_count > 1:
            ...
            if int(
                ...
                distribute_count = int(
                    fetched_volume.brick_count) / replica_count
                diff = int(
                ...
                if len(self.parameters["Volume.bricks"]) != distribute_count:
                    msg = "Insufficient Bricks for increasing replica" + \
                          " count distribute-sets: %d, bricks-provided: %d" % (
                              distribute_count,
                              len(self.parameters["Volume.bricks"])
                          )
                else:
                    # check if all distribute sets have required number of
                    # bricks
                    for brick_set in self.parameters["Volume.bricks"]:
                        if len(brick_set) != diff:
                            msg = "Incorrect number of bricks provided" \
                                  " for increasing replica count" + \
                                  "Each replica set needs %s. But %s given" % (
                                      diff, len(brick_set)
                                  )
                            break
            # check if user is trying to reduce the replica count using
            # expand vol
            elif int(
                    self.parameters.get(
                        "Volume.replica_count",
                ...
                msg = "Can't reduce the replica-count using"\
                      " expand volume. %d < %d" % (
                          self.parameters["Volume.replica_count"],
                          replica_count
                      )
            # user is trying to add new distribute set and we have to check
            # if each distribute set has requried number bricks
            # (equal to replica_count)
            else:
                for brick_set in self.parameters["Volume.bricks"]:
                    if len(brick_set) != replica_count:
                        msg = "Incorrect number of bricks provided for " + \
                              "Expand volume Each replica set needs %s " + \
                              "bricks. But %s given" % (
                                  replica_count, len(brick_set)
                              )
                        break
        # user is trying to expand dispersed volume, check if he is providing
        # enough bricks(k+m) for the new sub-volume
        elif disperse_count > 1:
            sub_vol_brick_count = disperse_count
            for brick_set in self.parameters["Volume.bricks"]:
                if len(brick_set) != sub_vol_brick_count:
                    msg = "Incorrect number of bricks provided for " + \
                          "Expand volume Each disperse sub-vol needs" + \
                          "%s bricks. But %s given" % (
        ...
        if msg:
            ...
            logger.log(
                "info",
                NS.publisher_id,
                {"message": "Inputs for expand volume "
